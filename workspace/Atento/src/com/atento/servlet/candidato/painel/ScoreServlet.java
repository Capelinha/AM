		...
		if (cookies != null) {
			...
			if ((idCand = cookiesH.get("idCandidato")) != null && (idSessao = cookiesH.get("idSessao")) != null) {
				...
				if (score >= (maxScore * 0.5)) {
					Candidato c = cDAO.redesSociais(idCandidato);
					...
					try {
						...
						Mensagem m = new Mensagem("Sucesso", "Parabens, você se candidatou a essa vaga. Existem provas pendentes.", "PROVAS", "provas");
					...
					RequestDispatcher dispatcher = request.getRequestDispatcher("WEB-INF/mensagem.jsp");
					Mensagem m = new Mensagem("Não compativel", "Desculpe mas você não compativel para essa vaga", "VOLTAR", "javascript:history.back()");
					request.setAttribute("mensagem", m);
